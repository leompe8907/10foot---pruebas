VODDetail = (function (Events) {
    ...
    $.extend(true, VODDetail, Events, {
		...
		setVodContent: function(vod) {
			...
			img = img == null || img.length == 0 ? vod.backgroundImageURL : img;
			img = img == null || img.length == 0 ? "" : img;
		...
		getHTMLEpisode: function(seasonId, episode) {
			...
			var image = image = $(".vod-serie-picture").attr("src");

			if (episode.image1Id != null) {
				image = this.vodObject.baseImageUrl.replace("{id}", episode.image1Id);
			} else if (episode.image2Id != null) {
				image = this.vodObject.baseImageUrl.replace("{id}", episode.image2Id);
			} else if (episode.image3Id != null) {
				image = this.vodObject.baseImageUrl.replace("{id}", episode.image3Id);
			}
